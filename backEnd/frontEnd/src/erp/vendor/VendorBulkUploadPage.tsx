...
import { BulkVendorPreviewResponse, previewVendorBulk, uploadVendorBulk } from "../api";

const TEMPLATE_HEADERS = ["공급사코드", "공급사명"];
...
function downloadTemplate() {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "대량공급사");
    XLSX.writeFile(wb, "대량공급사_양식.xlsx");
}
...
export default function VendorBulkUploadPage() {
    ...
    const [preview, setPreview] = useState<BulkVendorPreviewResponse | null>(null);
    ...
    const handlePreview = async () => {
        ...
        try {
            setPreview(await previewVendorBulk(file));
        } catch (e: any) {
    ...
    const handleConfirm = async () => {
        ...
        try {
            const res = await uploadVendorBulk(file);
            if (res.failRows > 0) {
            ...
            alert(`공급사 등록 완료: ${res.successRows}건 저장`);
            navigate("/erp/vendor");
        } catch (e: any) {
    ...
    return (
        <div>
            ...
            <div style={{ marginBottom: 10, display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                ...
                <button type="button" onClick={() => navigate("/erp/vendor")}>목록으로</button>
            </div>

            <div style={{ marginBottom: 10, color: "#666", fontSize: 13 }}>
                ※ 필수 컬럼: <b>공급사코드</b>, <b>공급사명</b>
                <br />
                ※ 공급사코드/공급사명은 기존 데이터 및 파일 내에서 <b>중복이면 오류</b>로 표시됩니다.
            </div>
