import { useRef, useState, type CSSProperties } from "react";
import { useNavigate } from "react-router-dom";
import * as XLSX from "xlsx";
import { BulkItemPreviewResponse, previewItemBulk, uploadItemBulk } from "../api";

const TEMPLATE_HEADERS = ["품목코드", "품목이름", "품목가격"];

function downloadTemplate() {
    const aoa: (string | number)[][] = [
        TEMPLATE_HEADERS,
        ["ITEM-001", "샘플품목", 1000],
    ];
    const ws = XLSX.utils.aoa_to_sheet(aoa);
    ws["!cols"] = [{ wch: 14 }, { wch: 18 }, { wch: 12 }];
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "대량품목");
    XLSX.writeFile(wb, "대량품목_양식.xlsx");
}

export default function ItemBulkUploadPage() {
    const navigate = useNavigate();
    const fileInputRef = useRef<HTMLInputElement>(null);

    const [file, setFile] = useState<File | null>(null);
    const [preview, setPreview] = useState<BulkItemPreviewResponse | null>(null);
    const [loading, setLoading] = useState(false);
    const [saving, setSaving] = useState(false);
    const [errorMsg, setErrorMsg] = useState<string | null>(null);

    const readErr = (e: any, fallback: string) =>
        e?.response?.headers?.["x-error-detail"] ?? e?.response?.data?.message ?? e?.message ?? fallback;

    const onSelectFile = (f: File | null) => {
        setFile(f);
        setPreview(null);
        setErrorMsg(null);
    };

    const handlePreview = async () => {
        if (!file) { alert("엑셀 파일을 먼저 선택해 주세요."); return; }
        setLoading(true); setErrorMsg(null); setPreview(null);
        try {
            setPreview(await previewItemBulk(file));
        } catch (e: any) {
            setErrorMsg(readErr(e, "미리보기 중 오류가 발생했습니다."));
        } finally {
            setLoading(false);
        }
    };

    const handleConfirm = async () => {
        if (!file || !preview?.confirmable) return;
        if (!window.confirm(`정상 ${preview.validRows}건을 등록합니다. 진행할까요?`)) return;
        setSaving(true); setErrorMsg(null);
        try {
            const res = await uploadItemBulk(file);
            if (res.failRows > 0) {
                const detail = res.errors.map((x) => `${x.row}행: ${x.message}`).join("\n");
                setErrorMsg("확정 중 검증 오류가 발생하여 저장되지 않았습니다.\n" + detail);
                await handlePreview();
                return;
            }
            alert(`품목 등록 완료: ${res.successRows}건 저장`);
            navigate("/erp/item");
        } catch (e: any) {
            setErrorMsg(readErr(e, "저장 중 오류가 발생했습니다."));
        } finally {
            setSaving(false);
        }
    };

    const cell: CSSProperties = { border: "1px solid #ccc", padding: "4px 6px", fontSize: 13 };
    const th: CSSProperties = { ...cell, background: "#f2f2f2", whiteSpace: "nowrap" };

    return (
        <div>
            <h2>대량 품목 엑셀 업로드</h2>

            <div style={{ marginBottom: 10, display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                <button type="button" onClick={downloadTemplate}>양식 다운로드</button>
                <input ref={fileInputRef} type="file" accept=".xlsx,.xls"
                    onChange={(e) => onSelectFile(e.target.files?.[0] ?? null)} />
                <button type="button" onClick={handlePreview} disabled={!file || loading}>
                    {loading ? "미리보기 중..." : "미리보기"}
                </button>
                <button type="button" onClick={() => navigate("/erp/item")}>목록으로</button>
            </div>

            <div style={{ marginBottom: 10, color: "#666", fontSize: 13 }}>
                ※ 필수 컬럼: <b>품목코드</b>, <b>품목이름</b>, <b>품목가격</b>
                <br />
                ※ 품목코드/품목이름은 기존 데이터 및 파일 내에서 <b>중복이면 오류</b>로 표시됩니다.
            </div>

            {errorMsg && (
                <div style={{ marginBottom: 10, padding: 8, background: "#fdecea", border: "1px solid #e74c3c", color: "#a93226", whiteSpace: "pre-wrap" }}>
                    {errorMsg}
                </div>
            )}

            {preview && (
                <>
                    <div style={{ marginBottom: 8, display: "flex", gap: 16, alignItems: "center", flexWrap: "wrap" }}>
                        <span>전체 <b>{preview.totalRows}</b>건</span>
                        <span style={{ color: "#2e7d32" }}>정상 <b>{preview.validRows}</b>건</span>
                        <span style={{ color: "#c0392b" }}>오류 <b>{preview.errorRows}</b>건</span>
                        <button type="button" onClick={handleConfirm} disabled={!preview.confirmable || saving}
                            style={{ marginLeft: "auto", padding: "6px 14px" }}
                            title={preview.confirmable ? "" : "오류 행이 있어 확정할 수 없습니다."}>
                            {saving ? "저장 중..." : "확정(저장)"}
                        </button>
                    </div>

                    {!preview.confirmable && preview.errorRows > 0 && (
                        <div style={{ marginBottom: 8, color: "#c0392b", fontSize: 13 }}>
                            ※ 오류 행이 있어 저장할 수 없습니다. 빨간색 행을 수정한 뒤 다시 업로드해 주세요.
                        </div>
                    )}

                    <table style={{ borderCollapse: "collapse", width: "100%" }}>
                        <thead>
                            <tr>
                                <th style={th}>행</th>
                                <th style={th}>품목코드</th>
                                <th style={th}>품목이름</th>
                                <th style={th}>품목가격</th>
                                <th style={th}>결과</th>
                            </tr>
                        </thead>
                        <tbody>
                            {preview.rows.map((r) => (
                                <tr key={r.rowNo} style={{ background: r.valid ? undefined : "#fdecea" }}>
                                    <td style={{ ...cell, textAlign: "center" }}>{r.rowNo}</td>
                                    <td style={cell}>{r.itemCode ?? ""}</td>
                                    <td style={cell}>{r.itemName ?? ""}</td>
                                    <td style={{ ...cell, textAlign: "right" }}>{r.standardPrice ?? ""}</td>
                                    <td style={{ ...cell, color: r.valid ? "#2e7d32" : "#c0392b" }}>
                                        {r.valid ? "정상" : r.error}
                                    </td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                </>
            )}
        </div>
    );
}
